...
CanMotorLF::CanMotorLF(std::string canName)
    : mCanName(canName)
{
...
void CanMotorLF::CanFunction()
{
    switch (sharedMemory->canLFState)
    {
    case CAN_NO_ACT:
    {
        usleep(10);
    ...
    case CAN_MOTOR_ON:
    {
        ...
        readEncoder();
        sharedMemory->motorLFState = true;
        break;
    }
    case CAN_INIT:
    {
        canInit();
        sharedMemory->canLFState = CAN_READ_ERROR;
        break;
    }
    case CAN_MOTOR_OFF:
    {
...
void CanMotorLF::canInit()
{
    ...
    sharedMemory->canLFStatus = true;
}

void CanMotorLF::canSend(int motorIndex, const u_int8_t* data)
{
...
void CanMotorLF::canRead()
{
    read(mSock, &mFrame, sizeof(mFrame));
}

void CanMotorLF::readEncoder()
{
...
void CanMotorLF::readMotorErrorStatus()
{
    for (int motorIndex = 0; motorIndex < MOTOR_NUM_PER_CAN; motorIndex++)
    {
